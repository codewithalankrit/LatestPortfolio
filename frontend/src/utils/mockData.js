export const mockProjects = [
  {
    id: 1,
    title: "CryptoExchange — Real-Time Cryptocurrency Tracker & Converter",
    shortDescription:
      "Track live crypto prices, view detailed coin data, and convert currencies instantly with CryptoExchange.",
    description:
      "CryptoExchange is your one-stop platform for real-time cryptocurrency tracking and conversion. Stay updated with the latest prices of major cryptocurrencies like Bitcoin, Ethereum, and many others. The sleek, user-friendly interface allows you to view detailed coin stats, market trends, and convert between cryptocurrencies and fiat in seconds. Whether you're a trader or a crypto enthusiast, CryptoExchange offers the tools you need to stay informed and make smart decisions — all in one place.",
    technologies: [
      "React",
      "TypeScript",
      "Vite",
      "React Router",
      "Tailwind CSS",
      "React Query",
      "shadcn UI",
      "Radix UI",
      "Lucide React",
      "React Hook Form",
      "Zod",
      "ESLint",
      "PostCSS",
      "Autoprefixer",
    ],
    images: ["/Tradepro.PNG"],
    liveLink: "https://cryptoexchangenew.vercel.app/",
    githubLink: "https://github.com/codewithalankrit/CryptoExchange",
    featured: true,
  },
  {
    id: 2,
    title:
      "3D Portfolio by Alankrit — Interactive Showcase of 3D Modeling Work",
    shortDescription:
      "A visually engaging 3D portfolio presenting 3D Modellers creative projects in modeling, animation, and digital design.",
    description:
      "This interactive 3D portfolio showcases the work of a talented 3D modeler and digital artist. Built with a sleek and modern interface, the site highlights a range of creative projects — from character designs and environments to animations and detailed modeling. The portfolio not only reflects artistic skill but also technical proficiency, providing a unique experience for visitors to explore 3D Modellers work in a dynamic, visually-rich format. Perfect for potential clients, collaborators, or anyone interested in high-quality 3D design.",
    technologies: ["HTML", "CSS", "Javascript"],
    images: ["/3d_portfolio.PNG"],
    liveLink: "https://codewithalankrit.github.io/3d_Portfolio/",
    githubLink: "https://github.com/codewithalankrit/3d_Portfolio",
    featured: true,
  },
  {
    id: 3,
    title: "Nintendo UI Concept — Clean, Modern Interface Inspired by Nintendo",
    shortDescription:
      "A sleek and responsive Nintendo UI clone that captures the nostalgic and modern feel of the Nintendo ecosystem.",
    description:
      "This project is a visually rich and interactive Nintendo UI concept designed to replicate the look and feel of Nintendo's user interface. It features smooth animations, a card-based layout, and responsive design, offering users a taste of how a modern Nintendo dashboard might look on the web. Whether you're exploring for design inspiration or evaluating frontend skills, this project showcases attention to UI detail, layout design, and polished styling — a creative take on blending nostalgia with modern web standards.",
    technologies: ["React", "Javascript", "Tailwind CSS", "Node.js", "Vite"],
    images: ["/StardewValley.PNG"],
    liveLink: "https://codewithalankrit.github.io/Nintendo_UI/",
    githubLink: "https://github.com/codewithalankrit/Nintendo_UI",
    featured: true,
  },
  {
    id: 4,
    title: "EventMaster - All-in-One Event Management Platform",
    shortDescription:
      "An elegant and responsive event management UI built for effortless planning, smart budgeting, guest tracking, and seamless execution.",
    description:
      "EventMaster is a modern, user-friendly frontend for a comprehensive event management platform designed to simplify the entire event planning process. Whether it's a wedding, corporate retreat, or birthday party, EventMaster provides everything you need to plan and manage events with confidence. The platform features a powerful dashboard, real-time budget tracking, RSVP and guest management, smart planning tools, vendor network integration, and advanced analytics. With clean UI/UX, responsive design, and clear visual feedback, it's ideal for planners and organizations of all sizes.",
    technologies: [
      "React",
      "Tailwind CSS",
      "Radix UI",
      "React Router",
      "React Hook",
      "Zod",
      "Lucide React",
      "Craco",
    ],
    images: ["/EventMaster.PNG"],
    liveLink: "https://frontend-virid-five-64.vercel.app/",
    githubLink: "https://github.com/codewithalankrit/EventMaster",
    featured: true,
  },
  {
    id: 5,
    title: "RAID Studio Architect — Modern SPA for Architecture Firm",
    shortDescription:
      "Single‑page site for an architecture studio with smooth scrolling, animated sections, galleries, and basic routing.",
    description:
      "Client project for RAID Studio (raidstudio.in). A modern single‑page website built with Vite, React 18, TypeScript, Tailwind CSS, and shadcn/ui (Radix UI primitives). It features smooth section scrolling, subtle animations, image gallery sections, and lightweight routing for legal pages and project details. The site emphasizes performance, accessibility, and a clean visual system driven by Tailwind tokens. Build is static and deploy‑ready for any CDN or static host.",
    technologies: [
      "React 18",
      "TypeScript",
      "Vite 5",
      "Tailwind CSS",
      "shadcn UI",
      "Radix UI",
      "React Router",
      "ESLint",
      "PostCSS",
      "Autoprefixer",
      "Recharts",
      "three.js",
      "@react-three/fiber",
      "@react-three/drei",
    ],
    images: ["/RaidStudio.PNG"],
    liveLink: "https://raidstudio.in",
    githubLink: "https://github.com/codewithalankrit/RaidStudio",
    featured: true,
  },
  {
    id: 6,
    title: "NovaFlow — Fake SaaS Startup Landing Page",
    shortDescription:
      "A modern, high-impact SaaS landing page concept designed to showcase conversion-focused UI, clean layouts, and premium frontend aesthetics — built without any backend.",
    description:
      "NovaFlow is a frontend-only SaaS startup landing page concept created to demonstrate advanced UI/UX skills and modern web design trends. The project focuses on a clear value proposition, strong visual hierarchy, conversion-optimized sections, and smooth interactions and spacing. The site simulates a real SaaS product with hero messaging, feature highlights, product previews, testimonials, and call-to-action blocks. Everything is handled on the frontend with no backend or database, keeping it lightweight, fast, and ideal for startups, demos, or design showcases. It was crafted to mirror real-world SaaS design standards common in high-growth startups today.",
    technologies: [
      "HTML5",
      "CSS3",
      "JavaScript",
      "Tailwind CSS",
      "React",
      "Vite",
      "Responsive Design",
      "Modern UI/UX Principles",
      "Frontend-only Architecture",
    ],
    images: ["/SAAS1.png"],
    liveLink: "https://flowbyte-workflow-automation-hub-ma.vercel.app/",
    githubLink: "https://github.com/codewithalankrit/FakeSaasWebsite",
    featured: true,
  },
];

export const mockSkills = [
  {
    category: "Frontend",
    skills: [
      { name: "React", icon: "⚛️" },
      {
        name: "TypeScript",
        icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/typescript/typescript-original.svg",
      },
      {
        name: "JavaScript",
        icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/javascript/javascript-original.svg",
      },
      { name: "HTML/CSS", level: 90, icon: "🎨" },
      {
        name: "Tailwind CSS",
        icon: "https://www.svgrepo.com/show/374118/tailwind.svg",
      },
      {
        name: "Bootstrap CSS",
        icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/bootstrap/bootstrap-original.svg",
      },
      {
        name: "Three.js",
        icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/threejs/threejs-original-wordmark.svg",
      },
      {
        name: "Shadcn UI",
        icon: "https://raw.githubusercontent.com/shadcn/ui/main/apps/www/public/apple-touch-icon.png",
      },
    ],
  },
  {
    category: "Tools & Others",
    skills: [
      {
        name: "Git",

        icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg",
      },
      {
        name: "Figma",

        icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/figma/figma-original.svg",
      },
      {
        name: "Adobe XD",

        icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/xd/xd-plain.svg",
      },
      {
        name: "Photoshop",

        icon: "https://cdn.jsdelivr.net/gh/devicons/devicon/icons/photoshop/photoshop-plain.svg",
      },
    ],
  },
];

export const mockPersonalInfo = {
  name: "Alankrit Aggarwal",
  title: "UI/UX Designer & Frontend Developer",
  email: "codewithalankrit@gmail.com",
  phone: "+91 99586 27696",
  location: "Noida, India",
  bio: "Passionate UI/UX designer and frontend developer with a love for creating beautiful, functional, and user-centered digital experiences. I specialize in modern web technologies and have a keen eye for design details.",
  resumeUrl: "/LatestResume.png",
  social: {
    linkedin: "https://www.linkedin.com/in/alankrit-aggarwal-b2968122a/",
    github: "https://github.com/codewithalankrit",
    instagram: "https://www.instagram.com/modernuibyaa/",
  },
};
